...
function VaultShellIsland() {
  ...
  const introPointerRef = useRef<{ id: number; lastY: number; moved: boolean } | null>(null);
  ...
  const navigateToVaultSeal = (stepIndex: number) => {
    if (cinematicRequested || !progressionHydrated || !introComplete || state.unlocked) {
      return;
    }
  ...
  const handleIntroPointerDown = (event: PointerEvent<HTMLElement>) => {
    ...
    introPointerRef.current = { id: event.pointerId, lastY: event.clientY, moved: false };
    try {
  ...
  const handleIntroPointerUp = (event: PointerEvent<HTMLElement>) => {
    const pointer = introPointerRef.current;
    if (pointer?.id === event.pointerId && pointer.moved) {
      // Prevent the compatibility click generated by the completed swipe.
      event.preventDefault();
    }
    clearIntroPointer(event);
  };
